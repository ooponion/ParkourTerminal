package parkourterminal.shader;
...
public class ShaderManager
{
    ...
    public ShaderManager(IResourceManager resourceManager, String programName) throws JsonException, IOException {
        ...
        ResourceLocation resourcelocation = new ResourceLocation("parkourterminal", "shaders/program/" + programName + ".json");
    ...
    /**
     * gets a parkourterminal.shader uniform for the name given. null if not found.
     */
    ...
    /**
     * gets a parkourterminal.shader uniform for the name given. if not found, returns a default not-null value
     */
    ...
    private void setupUniforms()
    {
        ...
        for (int j = 0; i < this.samplerNames.size(); ++j)
        {
            ...
            if (k == -1)
            {
                logger.warn("Shader " + this.programFilename + "could not find sampler named " + s + " in the specified parkourterminal.shader program.");
        ...
        for (ShaderUniform shaderuniform : this.shaderUniforms)
        {
            ...
            if (l == -1)
            {
                logger.warn("Could not find uniform named " + s1 + " in the specified" + " parkourterminal.shader program.");
            }
            else
            {
    ...
    /**
     * adds a parkourterminal.shader sampler texture. if it already exists, replaces it.
     */
