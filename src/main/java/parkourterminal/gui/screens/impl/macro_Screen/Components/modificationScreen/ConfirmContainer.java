...
public class ConfirmContainer extends Container{
    ...
    public ConfirmContainer(int x,int y,int width,int height){
        super(new Margin(0),new Padding(width/9,height*2/3,0,0),new noWarpLinearLayout(LayoutDirection.HORIZONTAL,width/9), ScrollDirection.VERTICAL);
        setSize(width, height);
        setPosition(x,y);
        cancelButton=new CustomButton(x,y,width/3, height/3,0xee7f9ba7,0xee2e96c3,4,"cancel");
        deleteButton=new CustomButton(x,y,width/3, height/3,0xee7f9ba7,0xee2e96c3,4,"delete");
    ...
    public void draw(int mouseX, int mouseY, float partialTicks) {
        if(isEnabled()){
            ...
            RenderTextHelper.drawCenteredString(mc.fontRendererObj,text,getX(),getY(),getWidth(), getPadding().top,0xFFf0b921,false);
        }
    }
    @Override
