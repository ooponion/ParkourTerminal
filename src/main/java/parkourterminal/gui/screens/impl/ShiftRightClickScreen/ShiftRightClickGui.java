...
//    private int maxScroll = 0;
    ...
    private Container coordLineContainer = new Container(new Margin(0,0,0,0),new Padding(25,20,25,20),new noWarpLinearLayout(LayoutDirection.VERTICAL,padding));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        fontRendererObj.drawString(String.valueOf(scrollBar.getContentOffset()),0,0,0xFFFFFFFF);
        ...
        if (heldItem != null && heldItem.hasTagCompound()) {
            ...
            if (nbt.hasKey("savedLocations")) {
                ...
                int fixedEntryWidth = coordLineContainer.getEntryWidth();
                // 添加到container
                coordLineContainer.Clear();
                for (int i = 0; i < savedLocations.tagCount(); i++) {
                    NBTTagCompound location = savedLocations.getCompoundTagAt(i);
                    coordLineContainer.addComponent(new CoordLine(location,width-50,fontRendererObj));
                }
                // 计算所有条目的总高度
                ...
                // 可视区域起始Y坐标（垂直居中）
                coordLineContainer.setY((int) -interpolatingOffset);
                // 绘制背景矩形
                int backgroundWidth = width-10; // 与各个条目的背景宽度一致
                float outerX = (width - backgroundWidth) / 2.0f;
                float outerY = coordLineContainer.getEntryTop() - 10;  // 第一个条目的顶部
                ...
                // 遍历每个条目进行绘制
//                int currentY = coordLineContainer.getEntryTop();
//                hoveredEntry = -1;
...
//                for (UIComponent component:coordLineContainer.getComponents()) {
//                    CoordLine coordLine=(CoordLine)component;
//
//                    int visibleWidth = coordLineContainer.getEntryWidth();
//                    boolean hasScrollbar = coordLine.hasScrollbar();
//                    //fontRendererObj.drawString(String.valueOf(fullTextWidth),0,0,0xFFFFFFFF);
//                    // 计算横向滚动偏移
...
//                    float boxY =  (coordLine.getY() - interpolatingOffset);
//
...
//                    fontRendererObj.drawStringWithShadow(coordLine.getDisplayText(), textX, textY, 0xFFFFFF);
//
//                    // 如果需要横向滚动，则绘制滚动条
////                    if (hasScrollbar) {
...
//                }
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
        ...
        for (UIComponent component:coordLineContainer.getComponents()){
            CoordLine coordLine =(CoordLine) component;
            if(coordLine.isMouseOver(mouseX,mouseY)){
                coordLine.getScrollBar().onClick(mouseX,mouseY);
                return;
            }
        }
...
//        hoveredScrollBarEntry = -1;
        for (UIComponent component:coordLineContainer.getComponents()){
            CoordLine coordLine =(CoordLine) component;
            coordLine.getScrollBar().onRelease();
        }
    }
}